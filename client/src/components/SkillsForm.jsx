...
const SkillsForm = ({data, onChange}) => {
    ...
    const addSkill = () => {
        if(newSkill.trim() && !data.includes(newSkill.trim())) {
            onChange([...data, newSkill.trim()]) //trim() removes the space from start and end of the string
            setNewSkill("")
        }
    }
    ...
    //function to handlle the ket press 
    ...
    <div className='space-y-4'>
        ...
        <div className='flex gap-2'>
            ...
            <button onClick={addSkill} disabled={!newSkill.trim} className='flex items-center gap-2 px-4 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed'>
                <Plus className='size-4'/> Add  {/* when we cant type in the input field then this button is disabled  */}
            </button>
        </div>
        ...
        <div className='bg-blue-50 p-3 rounded-lg'>
            <p className='text-sm text-blue-800'><strong>Tip : </strong> Add 8-12 relevant skills. Include both technical skills (programming langauges, tools) and soft skills (leadership, communication).</p>
        </div>
    </div>
  )
}
