...
const Dashboard = () => {
  ...
  //for resumes we are setting the different themse so thats why we require a color array
  ...
  const loadAllResumes = async () => {
    setAllResumes(dummyResumeData) // here we are using the dummy resume data from the assets
  }
  ...
  //whenever the components get loaded then this useEffect function will be called
  useEffect(() => {
    loadAllResumes() //this function will the add the data in the state
  },[]) 
  return (
    <div>
      ...
      {/* displaying the dummy resume data from the assets  */}
      ...
      {showCreateResume && (
        <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
          <div onClick={e => e.stopPropagation() } className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'> {/*e.stopPropagation() : It prevents the click event from bubbling up to parent elements. */}
